        ...
        <thead className="bg-gray-200">
          <tr>
            {headers.map((header, index) => (
              <th
                key={index}
                className={`px-6 py-4 border-r text-left border border-gray-300 font-semibold text-gray-700 ${index >= 5 ? 'font-normal' : ''}`}
              >
...
//     setGroupedData(groupDataByHeader(data, firstHeader));
//   }, [data, headers]);

//   const handleSort = (header, index) => {
//     const direction = sortDirection[header] === 'asc' ? 'desc' : 'asc';
//     setSortDirection({ [header]: direction });
//     setSortedData(sortData(data, header, direction, index));
//   };
